...
class PromptCompletionRating(dataset_base):
    ...
    prompt_completion: Mapped["PromptCompletion"] = relationship("PromptCompletion", back_populates="ratings")
    ...
    @classmethod
    def get(cls, dataset: "DatasetDatabase", completion: "PromptCompletion", facet: "Facet") -> "PromptCompletionRating | None":
        """
        Return the existing rating for *completion* and *facet*, if any.
        """

        session = dataset.get_session()
        return (session.query(cls).filter_by(prompt_completion_id=completion.id, facet_id=facet.id).one_or_none())

    @classmethod
    def set_rating(cls, dataset: "DatasetDatabase", completion: "PromptCompletion", facet: "Facet",
                   rating: int) -> "PromptCompletionRating":
        """
