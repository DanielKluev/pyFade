...
class PromptCompletion(dataset_base):
    ...
    logprobs: Mapped[list["PromptCompletionLogprobs"]] = relationship("PromptCompletionLogprobs", back_populates="prompt_completion", cascade="all, delete-orphan", lazy="select")
    sha256: Mapped[str] = mapped_column(String(64), nullable=False) # SHA256 hash of completion_text for deduplication. Allow different (prompt_revision_id, sha256) pairs.
    ...
    beam_token: Mapped[str | None] = mapped_column(nullable=True) # token at which beam tree was forked, if any
    ...
    @classmethod
    def get_or_create_from_llm_response(cls, dataset: "DatasetDatabase", prompt_revision: "PromptRevision", response: "LLMResponse") -> "PromptCompletion":
        """
