...
class WidgetExportTemplate(QWidget):
   ...
   def setup_ui(self) -> None:
      ...
      self.save_button.setStyleSheet("QPushButton { background-color: #3949AB; color: white; padding: 8px 16px; }")
      ...
      self.cancel_button.setStyleSheet("QPushButton { background-color: #9E9E9E; color: white; padding: 8px 16px; }")
   ...
   def add_selected_facet(self) -> None:
      ...
      if self._is_facet_present(facet_id):
         QMessageBox.information(self, "Facet Already Added", "This facet is already part of the export template.")
         return
   ...
   def _set_model_selection(self, models: list[str]) -> None:
      ...
      for index in range(self.model_list.count()):
         ...
         if not item:
            continue
         item.setCheckState(
            Qt.CheckState.Checked if item.text().strip().lower() in normalized_models else Qt.CheckState.Unchecked
         )
   ...
   def save_template(self) -> None:
      ...
      try:
         if self.template is None:
            ...
            self.log.debug("Updating export template id=%s with %s", self.template.id, form_data)
