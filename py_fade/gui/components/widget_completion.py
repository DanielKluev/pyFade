"""
Material-themed widget for viewing completions with multi-mode support and inline actions.

Supports both sample and beam display modes:
- Sample mode: Traditional completion display with rating controls, edit/resume buttons, and archive functionality  
- Beam mode: Simplified beam result display with save/pin/discard buttons and no rating controls

The widget automatically configures its UI based on the display_mode parameter, providing mode-appropriate 
controls while maintaining consistent Material Design styling throughout the app.

Key Features:
- Multi-mode architecture supporting "sample" and "beam" display modes
- Mode-specific action buttons (Discard, Edit, Save, Pin/Unpin, Resume, Archive)
- Enhanced signal architecture for parent-child communication
- Confirmation dialogs for destructive actions
- Smart visibility controls hiding inappropriate elements per mode
- Preserved prefill and beam token highlighting in text view

Signals:
- archive_toggled: Emitted when archive state changes (sample mode)
- resume_requested: Emitted when resume button clicked (sample mode) 
- evaluate_requested: Emitted when evaluate button clicked (sample mode)
- discarded: Emitted when discard button clicked (both modes)
- edit_requested: Emitted when edit button clicked (sample mode)
- saved: Emitted when save button clicked (beam mode)
- pinned: Emitted when pin state changes (beam mode)
"""
...
class CompletionFrame(QFrame):
    """
    Multi-mode completion display widget supporting both sample and beam display modes.
    
    Provides a unified interface for displaying completions with mode-specific controls:
    - Sample mode: Rating widget, edit/resume buttons, archive controls
    - Beam mode: Save/pin/discard buttons, simplified status display
    
    The widget automatically configures its layout and available actions based on the
    display_mode parameter, ensuring appropriate controls are shown for each mode.
    """
