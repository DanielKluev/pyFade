...
class InferenceProvidersManager:
    ...
    def __init__(self, models_configs: list[dict], ollama_models_dir: str|pathlib.Path|None = None, default_temperature: float = 0.7, default_top_k: int = 40):
            ...
            if 'gguf' in model_params:
                ...
                if gguf_path == "USE_OLLAMA_REGISTRY":
                    if not self.ollama_registry:
                        self.log.error(f"Model {model_id} requires Ollama registry, but no registry path provided.")
                        continue
                    if not ollama_id:
                        self.log.error(f"Model {model_id} requires Ollama registry, but no ollama_id provided.")
                        continue
                ...
                if is_llama_cpp_available:
                    ...
                    self.log.error(f"Model {model_id} requires llama_cpp provider, but llama-cpp-python is not installed.")
    ...
    def add_model(self, model_id: str, provider_key: str, provider_params: dict|None = None):
        ...
        if providers_map[provider_key] is None:
            self.log.error(f"Provider {provider_key} is not supported. Install required dependencies or configure correctly.")
            return
