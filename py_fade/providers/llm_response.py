...
class LLMResponse:
    ...
    full_history: list[
        dict
    ]  # system, user, and previous turn messages used to generate, but not including prefill
    ...
    @classmethod
    def from_completion_and_logprobs(
        cls, completion: "PromptCompletion", logprobs: "PromptCompletionLogprobs"
    ) -> "LLMResponse":
        """
        ...
        for lp in logprobs.logprobs:
            converted_logprobs.append(
                LLMPTokenLogProbs(
                    token=lp["token"], logprob=lp["logprob"], top_logprobs=lp.get("top_logprobs")
                )
            )
