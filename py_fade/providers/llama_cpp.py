...
try:
    ...
    print("Warning: Failed to import llama_cpp. Ensure llama-cpp-python is installed if you plan to use Llama.cpp models.")
    llama_cpp = None
...
class PrefillAwareLlamaCppInternal(BasePrefillAwareProvider):
    ...
    def load_model(self, model_id: str, gguf_file: str, n_gpu_layers: int = -1, verbose: bool = False, logits_all: bool = False, n_ctx: int = 1024) -> Optional["Llama"]:
        """
        If current loaded model is same as requested, return it.
        Else unload current model and load the new one.        
        """
        if self.current_model and self.current_model_id == model_id and self.current_model_gguf_file == gguf_file and self.current_model_logits_all == logits_all:
        ...
        self.log.info(f"Loading Llama model from {gguf_file} with n_gpu_layers={n_gpu_layers}, n_ctx={n_ctx}")
        try:
            model = llama_cpp.Llama(model_path=gguf_file, n_gpu_layers=n_gpu_layers, verbose=verbose, logits_all=logits_all, n_ctx=n_ctx)
    ...
    def generate(self, model_id:str, prompt: str, prefill: str|None = None, **kwargs) -> LLMResponse:
        ...
        model = self.load_model(model_id=model_id, gguf_file=gguf_file, n_gpu_layers=-1, verbose=True, logits_all=top_logprobs > 0, n_ctx=context_length)
        if not model:
            raise RuntimeError(f"Failed to load Llama model from {gguf_file}")

        self.log.info("-"*40)
        self.log.info(f"[ > ] Generating with model {model_id}, temperature={temperature}, top_k={top_k}, context_length={context_length}, max_tokens={max_tokens}, top_logprobs={top_logprobs}, prefill={'<yes>' if prefill else '<no>'}")
    ...
    def evaluate_completion(self, model_id: str, prompt: str, completion: str, **kwargs) -> list[LLMPTokenLogProbs]:
        ...
        self.log.info("-"*40)
        self.log.info(f"[ > ] Evaluating completion with model {model_id}, temperature={temperature}, top_k={top_k}, context_length={context_length}, max_tokens={max_tokens}, top_logprobs={top_logprobs}")
        ...
        logprobs_zipped = list(zip(logprobs_dict.get('tokens', []), logprobs_dict.get('token_logprobs', []), logprobs_dict.get('top_logprobs', [])))
    ...
    def _convert_chat_completion_logprobs(self, logprobs_dict: dict) -> list[LLMPTokenLogProbs]:
        ...
        if logprobs_dict:
            for token_info in logprobs_dict:
                ...
                logprobs.append(LLMPTokenLogProbs(token=token, logprob=logprob, top_logprobs=top_logprobs_list))
        return logprobs
    ...
    def _convert_simple_completion_logprobs(self, logprobs_dict: dict) -> list[LLMPTokenLogProbs]:
        ...
        for i in range(len(tokens)):
            ...
            logprobs_list.append(LLMPTokenLogProbs(token=token, logprob=logprob, top_logprobs=top_logprobs_list))
        return logprobs_list

    def mask_logprobs_by_str(self, logprobs: list[LLMPTokenLogProbs], mask_str: str, max_skip: int) -> list[LLMPTokenLogProbs]|None:
        """
