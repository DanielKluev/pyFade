...
class CompletionPrefix:
    ...
    next_token_logprobs: list[tuple[str, float]]|None = None # top N token logprobs for next token after prefix, if available
    ...
    @classmethod
    def create_from_eval(cls, prefix_text: str, eval_logprobs: list[LLMPTokenLogProbs]) -> "CompletionPrefix":
        """
...
class TextGenerationController:
    """
    Handles logic layer between GUI, dataset, and text generation models, gluing everything together.
    Controller is stateful and tied to a specific mapped_model, dataset and PromptRevision.
    """
    app: "pyFadeApp"
    ...
    def load_cache(self):
        ...
        """
        self.log.info(f"Loading cache for model {self.mapped_model.path} and prompt revision {self.prompt_revision.id}")
        ...
        self.log.info(f"Populated completions cache with {len(self.all_completions)} existing completions from dataset.")

    def beam_out_on_token_one_level(self, prefix: str, width: int, length: int, beam_tokens: list[tuple[str, float]]|None = None, on_beam_completed: Callable[[LLMResponse], None]|None = None, on_check_stop: Callable[[], bool]|None = None) -> list[LLMResponse]:
        """
        ...
        for token, logprob in beam_tokens[:width]:
            beam_token_prob = LLMPTokenLogProbs(token=token, logprob=logprob, top_logprobs=beam_tokens)
    ...
    def _expand_beam(self, beam_prefix: CompletionPrefix, beam_token_prob: LLMPTokenLogProbs, length: int) -> LLMResponse|None:
        """
